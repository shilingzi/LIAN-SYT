...
from .local_frequency_estimation import LocalFrequencyEstimationModule
...
class LMLTE(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None, hypernet_spec=None,
        ...
        # --- 新的高频模块，只做一次 sampling，输出 token_dim==C 的 t_hat ---
        C = self.encoder.out_dim
        self.freq_proj = nn.Linear(C, hidden_dim, bias=False)
        self.freq_estimator = LocalFrequencyEstimationModule(
            in_channels=C,
            latent_dim=hidden_dim,    # 任意你想要的中间维度
            token_dim=C               # 强制输出跟 feat 通道数一样
        )
    ...
    def gen_modulations(self, feat, cell=None):
        ...
        replacing the original high-frequency branch with LocalFrequencyEstimationModule.

        :param feat:  Latent feature map from encoder, shape [B, C, H, W]
        :param cell:  Cell areas, shape [B, H*W, 2]
        :return:      Latent modulations, shape [B, mod_dim, H, W]
        """
        bs, C, H, W = feat.shape
        feat_coord = self.feat_coord      # [B, 2, H, W]

        # Field radius (global normalized [-1,1])
        rx = 2.0 / H / 2
        ry = 2.0 / W / 2

        # 1) 用 LocalFrequencyEstimationModule 直接提取高频令牌 t_hat
        #    t_hat: [B, C, H, W]
        t_hat, mu_hat, logvar_hat = self.freq_estimator(feat)

        # 2) 准备 Key 分支（与原版一致）
        #    按照 local_size 构建偏移列表
        ...
        # 将 t_hat 转成 [B, H*W, C]，无需再投射
        t_flat = t_hat.view(bs, C, H*W).permute(0, 2, 1).contiguous()

        for vx, vy in v_lst:
            # --- coord_q / coord_k 及位置编码（与原版一致） ---
            ...
            key_s = (key_s * rel.unsqueeze(-1)).sum(dim=-2)          # [B, H*W, C//2]
            k_input = torch.cat([key_s, rel_encode, scale_], dim=-1) # [B, H*W, C//2+32+2]
            weight_k = self.imnet_k(k_input.view(bs*H*W, -1))        # [B*H*W, C]
            weight_k = weight_k.view(bs, H*W, C)                     # [B, H*W, C]

            # --- 聚合 preds_k & preds_v ---
            preds_k.append((key * weight_k).view(bs, H*W, -1))       # [B, H*W, C]
            # preds_v 直接使用 t_flat
            preds_v.append(t_flat)                                   # [B, H*W, C]

        # 堆叠
        preds_k = torch.stack(preds_k, dim=-1)  # [B, H*W, C, num_offsets]
        preds_v = torch.stack(preds_v, dim=-2)  # [B, H*W, num_offsets, C]

        # --- Query 分支（与原版一致） ---
        query = feat.view(bs, C, H*W).permute(0, 2, 1).contiguous()  # [B, H*W, C]
        query = self.imnet_q(query.view(bs*H*W, -1))               # [B*H*W, C]
        query = query.view(bs, H*W, C).unsqueeze(2)                # [B, H*W,1,C]

        # --- Attention 聚合 ---
        attn = (query @ preds_k) / self.softmax_scale               # [B, H*W,1,num_offsets]
        attn = attn.softmax(dim=-1)                                 # [B, H*W,1,num_offsets]
        inp_q = (attn @ preds_v).squeeze(2)                         # [B, H*W, C]

        # --- 投影到 hidden_dim ---
        inp_q_flat = inp_q.view(bs*H*W, C)                   # [B*H*W, C]
        inp_q_proj = self.freq_proj(inp_q_flat)              # [B*H*W, hidden_dim]

        # --- Hypernet & Render MLP ---
        mod = self.hypernet(inp_q_proj)                      # [B*H*W, mod_dim]
        mod = mod.view(bs, H, W, -1).permute(0, 3, 1, 2)     # [B, mod_dim, H, W]
