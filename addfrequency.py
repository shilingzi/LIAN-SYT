...
from models.local_frequency_estimation import LocalFrequencyEstimationModule
...
class LMLTE(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None, hypernet_spec=None,
        ...
        # 新的局部频率估计模块，用于预测 μ, σ 并采样出高频令牌 t_hat
        self.freq_estimator = LocalFrequencyEstimationModule(
            in_channels = self.encoder.out_dim,
        ...
        # 先计算 attention 中 value 的最终维度 value_dim，再去构造 v_proj
        # value_dim = feat_dim + t_hat_dim + non_local_attn_dim（如开启）
        self.value_dim = (
            self.encoder.out_dim              # 原始 feat
            + hidden_dim                      # t_hat 的通道数
            + (self.non_local_attn_dim if self.non_local_attn else 0)
        )
        ...
        self.v_proj = nn.Linear(2 * self.encoder.out_dim, self.value_dim, bias=False)
        # hypernet_in_dim 原来是 hidden_dim
        self.hyper_in_dim = hidden_dim
        self.hyper_in_proj = nn.Linear(self.value_dim, self.hyper_in_dim, bias=False)
    ...
    def positional_encoding(self,input,L): # [B,...,N]
        ...
        shape = input.shape
        freq = 2**torch.arange(L,dtype=torch.float32).cuda()
        freq  =  freq*np.pi # [L]
    ...
    def gen_modulations(self, feat, cell=None):
        """
        Generate latent modulations using the local-frequency-estimation module + attention.

        :param feat:  Latent feature map from encoder, shape [B, C, H, W]
        :param cell:  Cell areas, shape [B, H*W, 2]
        :return:      Latent modulations, shape [B, C', H, W]
        """
        bs, c, h, w = feat.shape
        ...
        # 1) 高频估计模块输出 t_hat
        t_hat, mu_hat, logvar_hat = self.freq_estimator(feat)  # [B, hidden_dim, H, W]

        # 2) 拼接 feat + t_hat (+ non_local_feat)
        p_hat = torch.cat([feat, t_hat], dim=1)  # [B, C+hidden_dim, H, W]
        if self.non_local_attn:
            p_hat = torch.cat([p_hat, self.non_local_feat], dim=1)
        D_value = p_hat.shape[1]
        value = p_hat.view(bs, D_value, h * w).permute(0, 2, 1)  # [B, H*W, D_value]

        # 3) key 仅用原始 feat
        key = feat.view(bs, c, h * w).permute(0, 2, 1)            # [B, H*W, C]

        # 4) 为 imnet_v 准备 t_flat，并做维度投射
        t_flat = t_hat.view(bs, t_hat.shape[1], h * w).permute(0, 2, 1)  # [B, H*W, hidden_dim]
        # imnet_v 输出
        weight_v_raw = self.imnet_v(
            t_flat.contiguous().view(bs * h * w, -1)
        ).view(bs, h * w, -1)                                             # [B, H*W, 2*C]
        # 映射到 D_value
        weight_v = self.v_proj(weight_v_raw)                             # [B, H*W, D_value]

        # 5) local ensemble 偏移列表
        if self.local_size == 1:
            v_lst = [(0, 0)]
        else:
            step = 4 - self.local_size
            v_lst = [(i, j) for i in range(-1, 2, step) for j in range(-1, 2, step)]

        preds_k, preds_v = [], []
        for vx, vy in v_lst:
            # --- coord_q / coord_k ---
            coord_q = feat_coord.view(bs, 2, h * w).permute(0, 2, 1)  # [B, H*W, 2]
            coord_k = coord_q.clone()
            coord_k[:, :, 0] += vx * rx / h
            coord_k[:, :, 1] += vy * ry / w

            # --- 相对坐标 & 位置编码 ---
            rel = coord_q - coord_k                                  # [B, H*W, 2]
            rel_coord = rel.clone()
            rel_coord[:, :, 0] *= h
            rel_coord[:, :, 1] *= w
            rel_encode = self.positional_encoding(rel_coord, L=8)    # [B, H*W, 32]

            # --- cell 信息 ---
            scale_ = cell[:, :h * w, :].clone()                      # [B, H*W, 2]
            scale_[:, :, 0] *= h
            scale_[:, :, 1] *= w

            # --- weight_k 部分 ---
            key_s = torch.stack(torch.split(key, 2, dim=-1), dim=-1) # [B, H*W, 2, C//2]
            key_s = key_s * rel.unsqueeze(-1)                        # [B, H*W, 2, C//2]
            key_s = key_s.sum(dim=-2)                                # [B, H*W, C//2]
            k_input = torch.cat([key_s, rel_encode, scale_], dim=-1) # [B, H*W, ...]
            weight_k = self.imnet_k(
                k_input.view(bs * h * w, -1)
            ).view(bs, h * w, -1)                                     # [B, H*W, C]

            # --- 聚合 preds_k, preds_v ---
            preds_k.append((key   * weight_k).view(bs, h * w, -1))   # [B, H*W, C]
            preds_v.append((value * weight_v).view(bs, h * w, -1))   # [B, H*W, D_value]

        # stack
        preds_k = torch.stack(preds_k, dim=-1)                       # [B, H*W, C, num_offsets]
        preds_v = torch.stack(preds_v, dim=-2)                       # [B, H*W, num_offsets, D_value]

        # 6) query from imnet_q
        query = feat.view(bs, c, h * w).permute(0, 2, 1).contiguous()
        query = self.imnet_q(
            query.view(bs * h * w, -1)
        ).view(bs, h * w, -1).unsqueeze(2)                           # [B, H*W, 1, C]

        # 7) attention + 融合
        attn = (query @ preds_k)                                     # [B, H*W, 1, num_offsets]
        attn = (attn / self.softmax_scale).softmax(dim=-1)
        inp_q = (attn @ preds_v).squeeze(2)                          # [B, H*W, D_value]

        # 8) 先投射到 hyper_in_dim，再进 hypernet
        inp_q_flat = inp_q.view(bs * h * w, -1)      # [B*H*W, value_dim]
        inp_q_proj = self.hyper_in_proj(inp_q_flat)  # [B*H*W, hyper_in_dim]
        mod = self.hypernet(inp_q_proj)              # [B*H*W, mod_dim]
        mod = mod.view(bs, h, w, -1).permute(0, 3, 1, 2)  # [B, C', H, W]
